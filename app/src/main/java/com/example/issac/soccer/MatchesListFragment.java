package com.example.issac.soccer;
...
public class MatchesListFragment extends ListFragment {
    ...
    private SoccerAdapter getAdapter(){
        SoccerAdapter adapter = new SoccerAdapter(getActivity(), R.layout.soccer_matches_layout, new ArrayList<Match>());

        try {
            JSONObject jsonObject = new JSONObject(getSON());
            ...
            for (int i=0; i<jsonArray.length(); i++){
                ...
                for (int j = 0; j < matches.length(); j++){
                    ...
                    m.marcador02 = unMatch.getInt("score2");
                    adapter.add(m);
